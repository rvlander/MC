function [nX,nY,signalx,signaly,tanB,psi] = resample_hw_cos(sampling,t0xs,t0ys,a,b,wx,wy,phix,phiy,c,X0,Y0)
...
for t=sampling
    ...
    signaly(i) = b(ky)*cos(wy(ky)*t+phiy(ky));
    signalx(i) = a(kx)*cos(wx(kx)*t+phix(kx));
    phi = phix(kx)-phiy(ky);
