function [d,J] = fullvecbigfun( theta, tdata , xdata, stdx, ydata, stdy, ...
...
for i=1:tailleX-1
    ...
    resx(indsx) = (xdata(indsx)-a(i)*cos(wx(i)*tdata(indsx)+phix(i)))/sqrt(stdx);
    restx((i-1)*(tailleX-1)+2) = is_wx*(wx(i)-wx0)/stdwx0;
    I = [i,tailleX+tailleY+i,2*tailleX+2*tailleY+i];
    dresx(indsx,I) = [-cos(wx(i)*tdata(indsx)+phix(i))/sqrt(stdx)   (tdata(indsx)*a(i)).*sin(wx(i)*tdata(indsx)+phix(i))/sqrt(stdx)  a(i)*sin(wx(i)*tdata(indsx)+phix(i))/sqrt(stdx)];
    drestx((i-1)*(tailleX-1)+2,I) = [ 0 is_wx 0];
end

for i=1:tailleY-1
    indsy = indy(i):indy(i+1);
    resy(indsy) = (ydata(indsy)-b(i)*cos(wy(i)*tdata(indsy)+phiy(i)))/sqrt(stdy);
    I = [tailleX+i,2*tailleX+tailleY+i,3*tailleX+2*tailleY+i];
    dresy(indsy,I) = [-cos(wy(i)*tdata(indsy)+phiy(i))/sqrt(stdy)   (tdata(indsy)*b(i)).*sin(wy(i)*tdata(indsy)+phiy(i))/sqrt(stdy)   b(i)*sin(wy(i)*tdata(indsy)+phiy(i))/sqrt(stdy)];
end
